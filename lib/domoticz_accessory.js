            ...
            case this.swTypeVal == Constants.DeviceTypeDimmer:
                {
                    ...
                    var isSpeaker = this.image && this.image.indexOf("Speaker") > -1;
                    var service = {};
                    var dimCharacteristic = {};
                    var powerCharacteristic = {};
		    if (isFan){
                        service = this.getService(Service.Fan);
                        dimCharacteristic = this.getCharacteristic(service, Characteristic.RotationSpeed);
                        powerCharacteristic = this.getCharacteristic(service, Characteristic.On);
		    } else if (isSpeaker) {
                        service = this.getService(Service.Speaker);
                        dimCharacteristic = this.getCharacteristic(service, Characteristic.Volume);
                        powerCharacteristic = this.getCharacteristic(service, Characteristic.Mute);
		    } else {
                        service = this.getService(Service.Lightbulb);
                        dimCharacteristic = this.getCharacteristic(service, Characteristic.Brightness);
                        powerCharacteristic = this.getCharacteristic(service, Characteristic.On);
		    }
			...
			if (!service) {
		        ...
		        var service = this.getService(Service.Lightbulb);
                    ...
                    if (isFan) {
                        ...
                        service.getCharacteristic(Characteristic.Volume).setProps({
                            minValue: 0,
                            maxValue: 100
                        }).on('set', this.setdValue.bind(this)).on('get', this.getdValue.bind(this));
                    } else {
